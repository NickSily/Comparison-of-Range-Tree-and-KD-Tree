        ...
        case DistributionType::GRID: {
            ...
            while (created < count && indices[0] < grid_size) {
                ...
                while (dim < dimensions) {
                    indices[dim]++;
                    if (indices[dim] < grid_size || dim == 0) break;
                    indices[dim] = 0;
                    dim--;
                }
...
BenchmarkResult benchmarkKDTree(
    ...
    switch (dist_type) {
        case DistributionType::UNIFORM_RANDOM: result.distribution = "Uniform Random"; break;
        case DistributionType::GAUSSIAN: result.distribution = "Gaussian"; break;
        case DistributionType::GRID: result.distribution = "Grid"; break;
        case DistributionType::CIRCLE: result.distribution = "Circle"; break;
        case DistributionType::SKEWED: result.distribution = "Skewed"; break;
    }
...
void writeAnalysisSummary(
    ...
    for (size_t i = 0; i < results.size(); i += 2) {
        if (i + 1 < results.size()) {
            ...
            double point_ratio = (range_result.point_search_time_existing / kd_result.point_search_time_existing +
                                  range_result.point_search_time_nonexisting / kd_result.point_search_time_nonexisting) / 2.0;
            ...
            double range_ratio = (range_result.small_range_query_time / kd_result.small_range_query_time +
                                 ...
                                 range_result.large_range_query_time / kd_result.large_range_query_time) / 3.0;
            avg_range_query_ratio += range_ratio;
            range_query_count++;
        }
    }
    
    if (construction_count > 0) avg_construction_ratio /= construction_count;
    if (point_search_count > 0) avg_point_search_ratio /= point_search_count;
    if (range_query_count > 0) avg_range_query_ratio /= range_query_count;
    ...
    file << "\nKD Tree:" << std::endl;
    file << "- Strengths: ";
    if (avg_construction_ratio > 1) file << "Faster construction, ";
    if (avg_point_search_ratio > 1) file << "Faster point search, ";
    file << "Supports nearest neighbor queries" << std::endl;
    file << "- Weaknesses: ";
    if (avg_range_query_ratio < 1) file << "Slower range queries, ";
    ...
    if (avg_construction_ratio < 1) file << "Faster construction, ";
    if (avg_point_search_ratio < 1) file << "Faster point search, ";
    if (avg_range_query_ratio < 1) file << "Faster range queries, ";
    file << "Optimized for orthogonal range queries" << std::endl;
    file << "- Weaknesses: ";
    if (avg_construction_ratio > 1) file << "Slower construction, ";
    if (avg_point_search_ratio > 1) file << "Slower point search, ";
    if (avg_range_query_ratio > 1) file << "Slower range queries, ";
    ...
    if (avg_construction_ratio > 1) file << "Construction speed is important, ";
    if (avg_point_search_ratio > 1) file << "Point searches are common, ";
    file << "Nearest neighbor queries are needed, ";
    file << "Memory usage is a concern" << std::endl;
    
    file << "- Use Range Tree when: ";
    if (avg_range_query_ratio < 1) file << "Range queries are the primary operation, ";
    ...
    for (const auto& dim : dimensions) {
        for (const auto& dist_type : distributions) {
            for (const auto& size : dataset_sizes) {
                ...
                switch (dist_type) {
                    case DistributionType::UNIFORM_RANDOM: std::cout << "uniform random distribution, "; break;
                    case DistributionType::GAUSSIAN: std::cout << "gaussian distribution, "; break;
                    case DistributionType::GRID: std::cout << "grid distribution, "; break;
                    case DistributionType::CIRCLE: std::cout << "circle distribution, "; break;
                    case DistributionType::SKEWED: std::cout << "skewed distribution, "; break;
                }
